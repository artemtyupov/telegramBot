...
namespace TelegramBot
{
    public class CGetDataState : IState
    {
        ...
        private static void ChangeInlineKeyboard(int idUser)
        {
            ...
            var idFolder = Convert.ToInt32(Database.MysqlSelect($"SELECT id FROM Folders WHERE idStorage = {idStorage} and Name = \"{Program._selectedButton}\"", Program.Conn));
            Program.Conn.Close();
        ...
        public async void ActionQuery(TelegramBotClient Bot, CallbackQuery callbackQuery)
        {
            ...
            var idMessage = Convert.ToInt32(Database.MysqlSelect($"SELECT idMessage FROM Files WHERE Name = \"{callbackQuery.Data}\" and idFolder = {idFolder}", Program.Conn));
            var idChat = Convert.ToInt64(Database.MysqlSelect($"SELECT idChat FROM Files WHERE idMessage = {idMessage} and idFolder = {idFolder}", Program.Conn));
